...
def app():
    ...
    with container:
        ...
        if 'all_activity_data' not in st.session_state or 'curr_activities_df_agg' not in st.session_state:
        ...
        curr_activities_pd_df_agg = st.session_state.curr_activities_df_agg
        curr_activities_pd_df = st.session_state.curr_activities_df

        # Get the starting and ending years of our data set
        start_year = curr_activities_pd_df_agg.sort_values('ACTIVITY_YEAR')['ACTIVITY_YEAR'].iat[0]
        end_year = curr_activities_pd_df_agg.sort_values('ACTIVITY_YEAR')['ACTIVITY_YEAR'].iat[-1]

        # Define a slider which will let us slice the data by activity year
        start_year_selected, end_year_selected = st.select_slider('',
        options=curr_activities_pd_df_agg.ACTIVITY_YEAR.unique(),
        value=(start_year, end_year))

        # Filter down our dataframes to the selection of the slider
        filtered_activities_df = curr_activities_pd_df_agg[curr_activities_pd_df_agg['ACTIVITY_YEAR'].between(start_year_selected, end_year_selected)]
        filtered_activities_all_df = curr_activities_pd_df[curr_activities_pd_df['ACTIVITY_YEAR'].between(start_year_selected, end_year_selected)]

        # Filter the dataframe and Get the total number of activities
        total_activities = filtered_activities_df['ACTIVITY_COUNT'].sum()
        ...
        all_metrics.append(style.metrics('weather source data', '2803', 'fa-cloud-sun', '#6B6C6C', 'weather_source_icon_logo.png'))
        ...
        graph_type = st.selectbox('',('Bar Chart - Altair', 'Line Chart - Altair', 'Area Chart - Altair', 'Map Scatterplot - PyDeck', 'Bar Chart - Bokeh'))

        if (graph_type == 'Bar Chart - Altair'):

            # Build out a bar chart with the dataframe
            charting.build_bar_chart(filtered_activities_df)

        elif (graph_type == 'Line Chart - Altair'):

            # Build out a bar chart with the dataframe
            charting.build_line_chart(filtered_activities_df)

        elif (graph_type == 'Area Chart - Altair'):

            # Build out a bar chart with the dataframe
            charting.build_area_chart(filtered_activities_df)
